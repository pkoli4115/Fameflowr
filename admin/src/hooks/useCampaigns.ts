...
export function useCampaigns(initialFilters: CampaignFilters = {}) {
  ...
  const loadPage = useCallback(
    async (reset: boolean, cursor?: QueryDocumentSnapshot<DocumentData> | null) => {
  ...
  // initial load
  useEffect(() => { loadPage(true, null); /* eslint-disable-next-line */ }, []);
  ...
  const onTogglePublish = useCallback(async (id: string, next: boolean) => {
